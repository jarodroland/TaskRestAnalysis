...
% numChannels = length(metaData.Channels);
% channelToElectrodeMap = metaData.Channels;     % maps channel number to electrode number for Task Data
carGroups = metaData.CARgroups;

% define variables
saveData = struct();
% samplingRate = metaData.samplefreq;   % task data sampling rate (not necessarily the resting data sampling rate)
...
saveData.restingSamplingRate = restingSamplingRate;
...
numChannels = size(restingSignal, 2);
...
for bandPassFrequencies = bandPassFrequencyList
    ...
    % lowPassFilter = designfilt('lowpassiir', 'FilterOrder', 4, 'PassbandFrequency', BLPFrequencies(2), 'SampleRate', restingSamplingRate);
...
%         colorbar()
%         fileOut = sprintf('%sFigures\\Correlation Matrix\\%s-Freq%03i-%03iHz-Seed%02i', dataDir, subjectID, bandPassFrequencies(1), bandPassFrequencies(2), channelToPlot);
