...
% % Nick's Gabor frequency and span lists
...
% variables to save in data file
saveData.subjectID = subjectID;
saveData.freqList = freqList;
saveData.samplingRate = samplingRate;
...
%     taskSignalReRefNotchedIQR = iqr(taskSignalReRefNotched{i}, 1);
    ...
    taskSignalNorm = bsxfun(@rdivide, bsxfun(@minus, taskSignalReRefNotched{i},  taskSignalReRefNotchedMean), taskSignalReRefNotchedStd);   % Z-score
    
    % calculate spectra for the whole file signal
    taskSpectra = gabor_cov_fitted(taskSignalNorm, freqList, samplingRate, freqSpan);                             % Zac's Gabor
...
saveData.itiMeanSpectra = itiMeanSpectra;
saveData.trialMeanSpectra = trialMeanSpectra;
...
% diffSpectra = bsxfun(@minus, bsxfun(@rdivide, trialMeanSpectra, itiTimeMeanSpectra), 1);   % subtract and divide the time averaged ITI spectra

saveData.diffSpectra = diffSpectra;
...
%     end
    ...
    ylim(axis, [-0.5 0.5]);
    hold on;
    plot([preMoveLength preMoveLength], [-0.5 0.5], 'k');   % movement onset marker
end
...
if(flags.saveFigures)
    ...
    print(figTimePowerPlot, fileOut, '-dpng');
    clear('fileOut');
...
saveData.preMoveLength = preMoveLength;
saveData.postMoveLength = postMoveLength;
